...
export default function UnifiedAdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'keys' | 'users' | 'changelog'>('keys');
...
👉 Truy cập và sử dụng tại: ${appUrl}`;
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col font-sans transition-colors duration-200">
      {/* Admin Header */}
      <header className="sticky top-0 z-30 backdrop-blur-md bg-white/85 dark:bg-slate-900/80 border-b border-slate-200 dark:border-slate-800/80 px-4 lg:px-8 py-3.5 flex flex-wrap items-center justify-between gap-4 shadow-sm dark:shadow-xl transition-colors">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
              ...
              >
                {currentUser.role === 'ADMIN' ? 'Quản Trị Viên (ADMIN)' : 'Cộng Tác Viên (CTV)'}
              </span>
            </h1>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Xin chào, <strong className="text-slate-800 dark:text-slate-200">{currentUser.name}</strong> ({currentUser.username})
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <a
            href="/"
            className="px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-medium border border-slate-200 dark:border-transparent transition flex items-center gap-1.5"
          >
            <Compass className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">Về Canvas</span>
          </a>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 lg:p-6 flex flex-col gap-6">
        ...
        {isStaff && (
          <div className="bg-gradient-to-r from-cyan-500/10 via-blue-500/10 to-indigo-500/10 border border-cyan-500/30 rounded-2xl p-4 sm:p-5 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          <div className="bg-white/80 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl p-4 flex items-center gap-4 shadow-sm dark:shadow-md transition-colors">
          ...
          <div className="bg-white/80 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl p-4 flex items-center gap-4 shadow-sm dark:shadow-md transition-colors">
            ...
            <div>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Key Đang Hoạt Động</p>
          ...
          {!isStaff && (
            <div className="bg-white/80 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl p-4 flex items-center gap-4 shadow-sm dark:shadow-md transition-colors">
              ...
              <div>
                <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Tổng Tài Khoản</p>
        ...
        {/* Tab Selector (Strictly visible to ADMIN only) */}
        {currentUser.role === 'ADMIN' && (
          <div className="flex items-center gap-2 border-b border-slate-200 dark:border-slate-800 pb-2">
            ...
            >
              ...
              <span>Quản Lý License Keys ({keys.length})</span>
            </button>
            ...
            >
              ...
              <span>Quản Lý Tài Khoản (Admin & CTV) ({userAccounts.length})</span>
            </button>
            ...
            >
              ...
              <span>Quản Lý Changelog ({changelogs.length})</span>
            </button>
          </div>
        )}

        {/* TAB 1: License Keys Management */}
        {(currentUser.role !== 'ADMIN' || activeTab === 'keys') && (
