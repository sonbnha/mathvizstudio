...
export const SavedCollection: React.FC<SavedCollectionProps> = ({
  ...
  return (
    <div className="w-full mt-3 rounded-xl border border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-slate-900/60 shadow-xs overflow-hidden transition-all shrink-0">
      ...
      {/* Lưới thumbnail nhỏ gọn khi mở rộng */}
      {isExpanded && (
        <div className="p-2.5 pt-0 border-t border-slate-100 dark:border-slate-800">
          {items.length === 0 ? (
            <div className="text-center py-5 text-xs text-slate-400 dark:text-slate-500">
              Chưa có hình vẽ nào được lưu vào bộ sưu tập.
            </div>
          ) : (
            <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-2 pt-2.5 max-h-48 overflow-y-auto pr-1">
              {items.map((item) => (
                <div
                  key={item.id}
                  onClick={(e) => {
                  ...
                  className="group p-1.5 rounded-lg border border-slate-200 dark:border-slate-700/60 hover:border-cyan-400 hover:shadow-xs bg-slate-50/60 dark:bg-slate-800/40 hover:bg-slate-100 dark:hover:bg-slate-800/90 cursor-pointer transition-all flex flex-col items-center relative"
                >
                  {/* Thumbnail mini h-16 sm:h-20 */}
                  <div className="w-full h-16 sm:h-20 rounded bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-800 flex items-center justify-center overflow-hidden p-1 relative [&>svg]:max-w-full [&>svg]:max-h-full [&>svg]:w-auto [&>svg]:h-auto">
                    {item.svgContent ? (
                      <div
                    ...
                    {onDeleteItem && (
                      <button
                        ...
                        className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 w-4 h-4 bg-black/70 hover:bg-rose-500 text-white rounded flex items-center justify-center text-[9px] transition-opacity cursor-pointer z-10"
                  ...
                  {/* Tên nhãn hình */}
                  <span
                    className="w-full text-[10px] font-medium text-slate-600 dark:text-slate-300 truncate text-center mt-1 group-hover:text-cyan-600 dark:group-hover:text-cyan-400"
                    title={item.title}
                  >
